...
class CXTextFieldViewController: CXBaseViewController {
    ...
    func otherControl() {
        //创建文本输入框
        let inputField = UITextField(frame: CGRect(x: 20, y: 80, width: 200, height: 30))
        inputField.borderStyle = UITextBorderStyle.roundedRect
        self.view.addSubview(inputField)
        
        //创建文本输入框
        let outField = UITextField(frame: CGRect(x: 20, y: 120, width: 200, height: 30))
        outField.borderStyle = UITextBorderStyle.roundedRect
        self.view.addSubview(outField)
        
        //创建文本标签
        let label = UILabel(frame: CGRect(x: 20, y: 160, width: 200, height: 30))
    ...
    func moreText() {
        ...
        Observable.combineLatest(inputField.rx.text.orEmpty, outField.rx.text.orEmpty) {
